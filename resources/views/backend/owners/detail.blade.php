                ...
                <form enctype="multipart/form-data" autocomplete="off" class="checkout_form" method="post">
                    ...
                    <div class="form-group">
                        ...
                        @endif
                            ...
                            <label name="name" style="font-size: 30px;">{{$room->name}}</label>
                            <br>
                        <div class="imgs" style="position: relative; bottom: 12px;">
                            <img src="{{asset('assets/images/location-point.png')}}" style="width: 20px; height: 20px;"><label name="city" >{{$room->address}}, {{$room->city}}, Indonesia</label>
                        </div>

                    <div class="form-group">
                        <br>
                        <img src="{{ url('uploads/file/'.$room->file) }}" style="width: 100%; height: 300px; margin-bottom: 5px;">
                    </div>
                    <div class="form-group">
                        <label class="label" for="descripton" style="font-size: 20px;">Description:</label>
                        <div class="input-group">
                            <textarea style="background-color: white; font-size: 20px;" class="form-control" readonly>{{$room->description}}</textarea>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="label" for="price" style="font-size: 20px;">Price:</label>
                        <div class="input-group">
                            <a style="font-size: 30px;color: #fa8231;font-weight: bolder">Rp. {{$room->price}}</a>
                        </div>
                    </div>
                    <div class="form-group">
